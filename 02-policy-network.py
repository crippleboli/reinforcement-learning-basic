...
# 3. 创建环境 进行游戏
# 3.1 初始化环境和智能体
env = gym.make("CartPole-v0")   # 创建平衡杆初始环境: 游戏的底层物理引擎、规则和奖惩机制
state = env.reset()             # S0: -0.05 到 0.05 之间 为这 4 个参数随机初始化
agent = Agent()
...
# 3.3 模拟计算策略梯度并更新网络
G = 1
J = -G * probs[action].log()
print("J: ", J)

agent.optimizer.zero_grad()
J.backward()  # 反向传播求导
agent.optimizer.step()  # 梯度下降更新参数

# 3.4 测试：检查相同状态下，该动作的概率是否发生变化
...
plot_loss(episode_list, return_list, "traditional-loss.pdf")